...
class StreamBus:
    ...
    Now uses asyncio.Event for zero-latency wakeups.
    """
    def __init__(self, maxsize: int = 500):
        self.queue = queue.Queue(maxsize=maxsize)
        self.total_dropped = 0
        self._new_packet_event = asyncio.Event()
    ...
    def push(self, packet: UnifiedInputPacket):
        ...
        try:
            ...
            # Wake up the consumer loop if it's waiting
            self._new_packet_event.set()
        ...
        while True:
            try:
                if not self.queue.empty():
                    ...
                    # Wait for a new packet without arbitrary polling sleeps
                    self._new_packet_event.clear()
                    await self._new_packet_event.wait()
            except Exception as e:
                print(f"[StreamBus Consumer Error] {e}")
                await asyncio.sleep(0.01) # Small safety backoff on error
